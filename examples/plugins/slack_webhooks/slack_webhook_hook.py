        ...
        def inner(cls):
            ...
            if "placeholders" in field_behaviors:
                ...
                field_behaviors["placeholders"] = {
                    _ensure_prefix(k): v for k, v in placeholders.items()
                }
        ...
        **kwargs,
        ...
        if http_conn_id:
            ...
            if slack_webhook_conn_id:
                raise AirflowException(
                    "You cannot provide both `slack_webhook_conn_id` and `http_conn_id`."
                )
            slack_webhook_conn_id = http_conn_id

        if not slack_webhook_conn_id and not webhook_token:
            raise AirflowException(
                "Either `slack_webhook_conn_id` or `webhook_token` should be provided."
            )
        if webhook_token:
    ...
    def send(
        ...
        **kwargs,
        ...
        body = {
            ...
            **{
                lip: self._resolve_argument(lip, kwargs.pop(lip, None))
                for lip in LEGACY_INTEGRATION_PARAMS
            },
        }
        if kwargs:
            warnings.warn(
